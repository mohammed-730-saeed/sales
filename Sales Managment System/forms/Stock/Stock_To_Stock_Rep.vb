...
Public Class Stock_To_Stock_Rep
    Public dt_Stock_To_Stock_Tbl As New DataTable
    Public Sub SelectAll_Stock_To_Stock_Tbl(dgv As DataGridView)
        Dim da As New SqlDataAdapter
        dt_Stock_To_Stock_Tbl.Clear()
        da = New SqlDataAdapter("select * from Stock_To_Stock_Tbl", Con)
        da.Fill(dt_Stock_To_Stock_Tbl)
        dgv.DataSource = dt_Stock_To_Stock_Tbl
    End Sub
    Public Sub DataGridViewHeaderText(ByVal DGV As DataGridView)
        If DGV.RowCount > 0 Then
            With DGV
                .Columns("Stock_Transfer_ID").HeaderText = "رقم التحويل"
                .Columns("Stock_Transfer_ID").width = 100
                .Columns("Money").HeaderText = "المبلغ المحول"
                .Columns("Money").width = 120
                .Columns("TransferDate").HeaderText = "تاريخ التحويل"
                .Columns("TransferDate").width = 150
                .Columns("StockFrom").HeaderText = "من"
                .Columns("StockFrom").width = 150
                .Columns("StockTo").HeaderText = "الى"
                .Columns("StockTo").width = 150
                .Columns("EmpName").HeaderText = "اسم الموظف"
                .Columns("EmpName").width = 150
                .Columns("Reason").HeaderText = "السبب"
                .Columns("Reason").width = 150
            End With
        End If
    End Sub

    Private Sub Stock_To_Stock_Rep_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        SelectAll_Stock_To_Stock_Tbl(DGV)
    ...
    Public Sub SEARCH2DATE(ByVal TransferDate As Date, ByVal TransferDate2 As Date)
        DT_SEARCH.clear()
        Dim CMDSEARCH As New SqlCommand(" SELECT * FROM Stock_To_Stock_Tbl WHERE TransferDate >= @TransferDate AND TransferDate <= @TransferDate2", con)
